...
'''
def img_kernel(img,kernel):
    m,n=img.shape
    imgK=np.zeros((m,n))
    for i in range(n-kernel+1):
        for j in range(m-kernel+1):
             a=img[i:i+kernel,j:j+kernel].flatten()
             a.sort()
             imgK[i:i + kernel, j:j + kernel]=a[int(kernel*kernel/2)]
    return imgK
...
def medianBlur(img, kernel, padding_way):
    ...
    if padding_way == "ZERO":
        img_extend = np.zeros((H + 2 * extend, W + 2 * extend))
        img_extend[extend:extend + H, extend:extend + W] = img
    elif padding_way=="REPLICA":
        for _ in range(extend):
            img_extend=one_extend(img)

    return img_kernel(img_extend,kernel)
#第二题：RANSAC
import cv2
def ransacMatching(A, B):
    return cv2.findHomography(A,B,cv2.RANSAC,4)[0]
#第三题：图像拼接
